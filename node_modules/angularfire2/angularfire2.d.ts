import { InjectionToken, NgZone } from '@angular/core';
import { Observable, Subscription } from 'rxjs';
import { FirebaseOptions, FirebaseAppConfig } from '@firebase/app-types';
export declare const FirebaseAppNameToken: InjectionToken<string | undefined>;
export declare const FirebaseOptionsToken: InjectionToken<FirebaseOptions>;
export declare const FirebaseAppConfigToken: InjectionToken<FirebaseAppConfig | undefined>;
export declare const RealtimeDatabaseURL: InjectionToken<string>;
export declare class FirebaseZoneScheduler {
    zone: NgZone;
    private platformId;
    constructor(zone: NgZone, platformId: Object);
    schedule(...args: any[]): Subscription;
    keepUnstableUntilFirst<T>(obs$: Observable<T>): Observable<T>;
    runOutsideAngular<T>(obs$: Observable<T>): Observable<T>;
}
